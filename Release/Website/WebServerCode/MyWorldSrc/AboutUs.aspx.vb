Partial Class AboutUs
 ...
 '*************************************************************************************************
 '* About Us Content display page. Use the Administration Web Content Management page to provide any
 ...
 Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
  ...
  If Trace.IsEnabled Then Trace.Warn("AboutUs", "Start Page Load")
  ...
  If IsNothing(Session("SSLStatus")) Then
   ...
   If Request.ServerVariables("HTTPS") = "off" Then          ' Security is not active and is required
    If Trace.IsEnabled Then Trace.Warn("AboutUs", "Https is off")
    ...
    If Trace.IsEnabled Then Trace.Warn("AboutUs", "Get location SQLCmd: " + SQLCmd)
    drServer = MyDB.GetReader("MySite", SQLCmd)
    If drServer.Read() Then
     If Trace.IsEnabled Then Trace.Warn("AboutUs", "ServerLocation = " + drServer("Parm2").ToString().Trim().ToUpper())
     If drServer("Parm2").ToString().Trim().ToUpper() = "LIVE" Then ' Security must be on if not testing
      drServer.Close()
      Session("SSLStatus") = True
      Response.Redirect("https://" + Request.ServerVariables("HTTP_HOST") + "/AboutUs.aspx")
     End If
    Else                                                     ' show error if not located
     If Trace.IsEnabled Then Trace.Warn("AboutUs", "Set https error: <br>Control value for 'Server Location' (ServerLocation) was not defined!")
  ...
  If Not IsPostBack Then                                    ' First time page is called setup
   ...
   If Trace.IsEnabled Then Trace.Warn("AboutUs", "Get Page Record Check: " + SQLCmd)
   drGetPage = MyDB.GetReader("MySite", SQLCmd)
   If Trace.IsEnabled And MyDB.Error Then Trace.Warn("AboutUs", "DB Error: " + MyDB.ErrMessage().ToString())
   If Not drGetPage.HasRows() Then
    drGetPage.Close()
    ' Create page entry
    SQLFields = "Name,Path"
    SQLValues = "'About Us'," + MyDB.SQLStr(Request.ServerVariables("URL"))
   ...
   If drGetPage.Read() Then
    ...
    If Trace.IsEnabled Then Trace.Warn("AboutUs", "Update Page AutoStart: " + SQLCmd)
    MyDB.DBCmd("MySite", SQLCmd)
    ...
    SQLCmd = "Select Title,Content From pagedetail " +
             ...
             "Order by SortOrder"
    If Trace.IsEnabled Then Trace.Warn("AboutUs", "Get Page Content: " + SQLCmd)
   ...
   If Not (Session.Count() = 0 Or Len(Session("UUID")) = 0) Then  ' Allow menu access while logged in
    ...
    End If
    If Trace.IsEnabled Then Trace.Warn("AboutUs", "Show Menu")

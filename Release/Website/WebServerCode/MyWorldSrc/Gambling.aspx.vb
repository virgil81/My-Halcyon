...
Partial Class Gambling
 ...
 Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
  ...
  If Trace.IsEnabled Then Trace.Warn("Gambling", "Start Page Load")
  ...
  If Not IsPostBack Then                                    ' First time page is called setup
   ...
   If Trace.IsEnabled Then Trace.Warn("Gambling", "Get Page Record Check: " + SQLCmd)
   drGetPage = MyDB.GetReader("MySite", SQLCmd)
   If Trace.IsEnabled And MyDB.Error Then Trace.Warn("Gambling", "DB Error: " + MyDB.ErrMessage().ToString())
   If Not drGetPage.HasRows() Then
    drGetPage.Close()
    ' Create page entry
    SQLFields = "Name,Path"
    SQLValues = "'Gambling'," + MyDB.SQLStr(Request.ServerVariables("URL"))
    SQLCmd = "Insert into pagemaster (" + SQLFields.ToString() + ") Values (" + SQLValues.ToString() + ")"
    If Trace.IsEnabled Then Trace.Warn("Gambling", "Insert Page record: " + SQLCmd)
    MyDB.DBCmd("MySite", SQLCmd)
    If Trace.IsEnabled And MyDB.Error Then Trace.Warn("Schools", "DB Error: " + MyDB.ErrMessage().ToString())
    ' Reload PageID
    SQLCmd = "Select PageID From pagemaster Where Path=" + MyDB.SQLStr(Request.ServerVariables("URL"))
    If Trace.IsEnabled Then Trace.Warn("Gambling", "Reload Page RecordID: " + SQLCmd)
    drGetPage = MyDB.GetReader("MySite", SQLCmd)
    If Trace.IsEnabled And MyDB.Error Then Trace.Warn("Gambling", "DB Error: " + MyDB.ErrMessage().ToString())
   End If
   ...
   If drGetPage.Read() Then
    ...
    If Trace.IsEnabled Then Trace.Warn("Gambling", "Update Page AutoStart: " + SQLCmd)
    MyDB.DBCmd("MySite", SQLCmd)
    ...
    SQLCmd = "Select Title,Content From pagedetail " +
             ...
             "Order by SortOrder"
    If Trace.IsEnabled Then Trace.Warn("Gambling", "Get Page Content: " + SQLCmd)
